...
const TECH_COLORS: Record<string, string> = {
    "AI": "#a855f7", "ML": "#8b5cf6", "Deep Learning": "#7c3aed",
    "Android": "#3ddc84", "IBM Watson": "#0f62fe", "Java": "#f89820",
    "Quantum": "#06b6d4", "Research": "#22d3ee",
    "TensorFlow": "#ff6f00", "IBM Cloud": "#0f62fe",
    "Ansible": "#e00", "Terraform": "#5c4ee5",
}
...
function ProjectCard({ project, index }: { project: typeof portfolioData.projects[0]; index: number }) {
    const tilt = useTilt({ max: 14, perspective: 900, scale: 1.04 })
    ...
    return (
        ...
        >
            <motion.div
                ...
                className="group glass border border-border rounded-2xl overflow-hidden h-full flex flex-col cursor-pointer depth-shadow relative"
            >
                {/* Color bar animates in on hover */}
                <div className="absolute top-0 left-0 right-0 h-[2px] w-0 group-hover:w-full transition-all duration-700 z-10"
                    style={{ background: `linear-gradient(90deg, ${primaryColor}, transparent 80%)` }} />

                {/* Radial glow follows mouse */}
                <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none"
                    style={{ background: `radial-gradient(circle at 50% 50%, ${primaryColor}12 0%, transparent 65%)` }} />

                <div className="p-6 flex flex-col h-full relative z-10">
                    <div className="flex items-start justify-between mb-4">
                        <Folder className="h-5 w-5 text-foreground/30 group-hover:text-primary transition-colors" />
                        <a href={project.link} target="_blank" rel="noopener noreferrer"
                            className="text-foreground/30 hover:text-primary transition-colors p-1.5 rounded-lg hover:bg-primary/10 -mr-1.5 -mt-1.5">
                            <ArrowUpRight className="h-4 w-4" />
                        </a>
                    </div>

                    <h3 className="font-bold font-mono text-foreground text-sm mb-2">{project.title}</h3>
                    <p className="text-sm text-foreground/50 leading-relaxed flex-grow mb-5">{project.description}</p>

                    <div className="flex items-center gap-4 text-[11px] font-mono text-foreground/40 mb-4">
                        <div className="flex items-center gap-1.5">
                            <span className="h-2.5 w-2.5 rounded-full" style={{ background: primaryColor }} />
                            {project.techStack[0]}
                        </div>
                        <div className="flex items-center gap-1"><Star className="h-3 w-3 text-yellow-400" />{stars}</div>
    ...
    return (
        <section id="projects" ref={sectionRef} className="py-32 relative overflow-hidden">
            ...
            <motion.div style={{ x: bgX }}
                className="absolute top-1/2 -translate-y-1/2 pointer-events-none select-none opacity-[0.025] whitespace-nowrap z-0"
                aria-hidden>
            ...
            <div className="container mx-auto max-w-6xl relative z-10">
                ...
                >
                    ...
                    <p className="text-foreground/40 font-mono text-xs mt-3">Hover for 3D interaction · Click for live demo</p>
                </motion.div>
                ...
                >
                    <motion.a
                        ...
                        className="group inline-flex items-center gap-3 px-8 py-4 rounded-2xl glass border border-border text-sm font-bold depth-shadow hover:depth-shadow-lg hover:border-primary/30 transition-all duration-300"
                    >
