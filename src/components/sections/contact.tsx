...
const socials = [
    ...
    { key: "website", label: "Website", color: "#fff", bg: "#0891b2", icon: <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10" /><line x1="2" y1="12" x2="22" y2="12" /><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z" /></svg> },
...
function MatrixRain() {
    ...
    useEffect(() => {
        ...
        const chars = "アイウエオカキクケコサシスセソ01234567890ABCDEF".split("")

        let raf: number
        const draw = () => {
            ctx.fillStyle = "rgba(4,4,15,0.07)"
            ctx.fillRect(0, 0, w, h)
            ctx.font = "12px JetBrains Mono, monospace"
            drops.forEach((y, i) => {
                const c = chars[Math.floor(Math.random() * chars.length)]
                ctx.fillStyle = i % 5 === 0 ? "#818cf840" : "#5046e525"
...
export function Contact() {
    ...
    return (
        <section id="contact" className="py-24 sm:py-32 min-h-[80vh] flex items-center relative overflow-hidden">
            {/* Matrix rain */}
            ...
            <div className="aurora-bg">
                ...
                <div className="aurora-2" style={{ opacity: 0.18 }} />
                <div className="aurora-3" style={{ opacity: 0.12 }} />
            </div>
            ...
            <div className="container mx-auto max-w-lg relative z-10 px-4">
                <motion.div
                    ...
                    className="glass-card glow-pulse card-accent-top p-7 sm:p-10 text-center depth-shadow-lg"
                >
                    {/* Icon */}
                    <motion.div
                        whileHover={{ rotate: 15, scale: 1.15 }}
                        transition={{ type: "spring", stiffness: 300 }}
                        className="h-14 w-14 sm:h-16 sm:w-16 rounded-2xl flex items-center justify-center mx-auto mb-5 sm:mb-6 shadow-xl"
                        style={{ background: "linear-gradient(135deg, var(--primary), var(--secondary))" }}
                    >
                        <MessageSquare className="h-6 w-6 sm:h-7 sm:w-7 text-white" />
                    </motion.div>

                    <h2 className="display text-2xl sm:text-3xl md:text-4xl mb-3">
                        Let&apos;s <span className="gradient-text">Collaborate</span>
                    </h2>
                    <p className="text-foreground/50 text-sm leading-relaxed mb-5 sm:mb-6 max-w-sm mx-auto">
                    ...
                    {/* Speaking badge */}
                    <div className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-xs sm:text-sm font-medium mb-6 sm:mb-8"
                        ...
                        {socials.map(({ key, label, color, bg, icon }) => {
                            ...
                            return (
                                <motion.a
                                    ...
                                    className="inline-flex items-center gap-2 h-9 sm:h-10 px-4 sm:px-5 rounded-xl text-xs sm:text-sm font-bold shadow-lg backdrop-blur-md"
                                    style={{
                                        color,
                                        background: `${bg}e0`,
                                        border: `1px solid ${bg}50`,
                                        boxShadow: `0 4px 16px ${bg}30`
                                    }}
                                >
                                    {icon} {label}
                                </motion.a>
                    ...
                    <p className="text-[10px] text-foreground/25 font-mono mt-6 sm:mt-8">
