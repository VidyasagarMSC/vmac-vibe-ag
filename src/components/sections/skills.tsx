...
const catConfig = [
    { icon: Layout, color: "#00f0ff", bg: "rgba(0, 240, 255, 0.1)" },
    { icon: Cpu, color: "#b600f8", bg: "rgba(182, 0, 248, 0.1)" },
    { icon: Database, color: "#0891b2", bg: "rgba(8, 145, 178, 0.1)" },
    { icon: Server, color: "#7c3aed", bg: "rgba(124, 58, 237, 0.1)" },
]

const awardCfg: Record<string, { icon: typeof Trophy; color: string }> = {
    "BCS Fellow": { icon: Trophy, color: "#f59e0b" },
    "Distinguished Architect": { icon: Star, color: "#0f62fe" },
    "Quantum-Safe": { icon: Zap, color: "#00f0ff" },
    "Most Admired": { icon: Award, color: "#a855f7" },
    "Globee": { icon: Award, color: "#f97316" },
    "Marquis": { icon: Star, color: "#94a3b8" },
    "IBM Outstanding": { icon: Trophy, color: "#0f62fe" },
    "DZone": { icon: Sparkles, color: "#c42127" },
    "Microsoft": { icon: Star, color: "#00a4ef" },
...
function NeuralCanvas() {
    ...
    useEffect(() => {
        ...
        const nodes = Array.from({ length: NODE_COUNT }, () => ({
            ...
            r: 2.5 + Math.random() * 2,
            color: Math.random() > 0.5 ? "#b600f8" : "#00f0ff",
        ...
        const draw = () => {
            ...
            nodes.forEach((n) => {
                ...
                ctx.fillStyle = n.color + "cc"; ctx.shadowBlur = 8; ctx.shadowColor = n.color
                ...
                <div className="grid lg:grid-cols-5 gap-8 lg:gap-10">
                    ...
                    <div className="lg:col-span-3 space-y-7 sm:space-y-10">
                        ...
                        <motion.div
                            ...
                            className="h-32 sm:h-36 rounded-2xl glass-card border border-border/60 overflow-hidden"
                        >
                            <NeuralCanvas />
                        </motion.div>
                    </div>
                    ...
                    >
                        <motion.div
                            ...
                            className="glass-card card-accent-top p-5 sm:p-7 depth-shadow-lg h-full"
                            style={{ transformPerspective: 1000 }}
                        >
                            <h3 className="font-bold font-heading text-base sm:text-lg mb-5 sm:mb-7 flex items-center gap-2">
                                <Award className="h-5 w-5 text-primary" /> Recognition
                            </h3>

                            {/* Top 2 featured awards */}
                            <div className="grid grid-cols-2 gap-2.5 mb-5 sm:mb-7 pb-5 sm:pb-6 border-b border-border/60">
                                {portfolioData.awards.slice(0, 2).map((a, i) => (
                                    <div key={i} className="text-center glass-panel p-3 sm:p-4 cursor-default">
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className="h-7 w-7 sm:h-8 sm:w-8 rounded-lg flex items-center justify-center shrink-0"
                                                ...
                                                <AIcon className="h-3 w-3 sm:h-3.5 sm:w-3.5" style={{ color: cfg.color }} />
                                            </div>
                                            <div className="min-w-0">
                                                <p className="text-xs font-semibold text-foreground group-hover:text-primary transition-colors leading-snug truncate">{award.title}</p>
                                                <p className="text-[9px] text-foreground/40 truncate">{award.issuer} · {award.year}</p>
                                            </div>
                                        </motion.div>
                                    )
                                })}
